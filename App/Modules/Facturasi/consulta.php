<?php
	...
	$con=conectar();
	$nom=$_POST["no"];
	$query="SELECT * FROM as_defactura WHERE nofactura like '%{$nom}%';";
	...
	if ($con->affected_rows>0){
		...
		while ($row=$result -> fetch_assoc()) {
			$msg=$msg."<tr>
							...
							<button class='btn btn-primary btn-md btn-xs' id='".$row["nofactura"]."&tipo=inversa' onClick='javascript:ver(this.id)'><i class='fa fa-eye'></i></button></center></td>
						</tr>";
		}
		$msg=$msg."</tbody></table>";
