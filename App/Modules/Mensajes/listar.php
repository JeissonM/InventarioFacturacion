			...
			if ($row["borrador"]==="NO"){
				if ($row["para"]==$_SESSION["id"]){
								if ($row["estado"]==="SI"){
									...
									$msg=$msg."<tr id='msjn'><td class='mailbox-name'><a href='leermsj.php?para=".$row["paran"]."&de=".$row["den"]."&asunto=".$row["asunto"]."&mensaje=".$row["mensaje"]."&fecha=".$row["fecha"]."&id=".$row["id"]."'>".$row["den"]."</a></td>
											<td class='mailbox-subject'><b>".$row["asunto"]."</b></td>
				                    		<td class='mailbox-date'>".$row["fecha"]."</td>
				                    		<td><button type='button' class='btn btn-danger btn-xs' id='".$row["id"]."' onclick='eliminar(this.id)'><i class='fa fa-trash-o'></i></button></td>		
									</tr>";
