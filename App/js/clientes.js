...
function guardar(){
    ...
    $.ajax({
        type:'POST',
        url: urlg + '/Modules/Clientes/nueva.php',
...
function listar() {
    $.ajax({
        type:'POST',
        url: urlg + '/Modules/Clientes/listar.php',
        data:{"si":"si"},
    }).done(function( msg ) {
        $('#tabla').html(msg);
        $('#table1').DataTable();
...
function consultar(){
  ...
  nom=nom.toUpperCase();
   $.ajax({
        type:'POST',
        url:urlg + '/Modules/Clientes/consulta.php',
        data:{"nombre":nom},
    }).done(function( msg ) {
        $('#result').html(msg);
        listar();
...
function eliminar(id){
  if (confirm("¿Esta seguro de eliminar el cliente?")){
      $.ajax({
        type:'POST',
        url: urlg + '/Modules/Clientes/eliminar.php',
        data:{"id":id},
      }).done(function( msg ) {
          //$('#result').html(msg);
          alert(msg);
...
function modificar(){
    ...
    $.ajax({
        type:'POST',
        url: urlg + '/Modules/Clientes/modificar.php',
        data:{"nit":nit, "nom":nom, "des":des, "dir":dir, "tel":tel, "mail":mail, "ciu":ciu},
    }).done(function( msg ) {
      var m="msj2";
        mensage(msg,"info",m);
